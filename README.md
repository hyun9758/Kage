...
### 1) 메인 페이지
- 캐릭터의 이미지와 정보를 업로드할 수 있다.
- 정보에는 이름, 나이, 성격, 배경 스토리, 소개글 등 다양하게 커스터마이징이 가능하다.
- 테마 색상을 지정하여 차별화하였다.
...
### 2) 캐릭터 목록 페이지
- 총 캐릭터 수, 좋아요와 공유 수, 조회수까지 확인이 가능하다.
- 캐릭터들은 카드 형식으로 목록에 뜨게 된다.
  자세히 보기를 눌렀을 때 사용자가 입력했던 정보를 그대로 확인 가능하며 작성자일 경우 버튼을 눌러 삭제 및 수정이 가능하다. 
...
### 5) AI 챗봇 
- 캐릭터 정보(이름, 성격, 배경 스토리)를 프롬프트로 활용한 AI 채팅 기능 구현하였다.
- 스트리밍 응답을 처리해 실시간 채팅 UI 구현하였다.
- API 연결 상태 확인 후 채팅 가능 여부를 안내한다.

|AI 챗봇|
|---|
|<img width="1865" height="989" alt="image" src="https://github.com/user-attachments/assets/01b8ab9f-1c53-49ef-b9a7-c4505a7182a8" />|
